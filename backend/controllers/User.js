...
exports.signup = (req, res) => {
  if ((regex.test(req.body.mdp) === true) && (mailRegex.test(req.body.mail) === true) && (regex.test(req.body.pseudo) === true)){
    ...
    .then(hash =>{
      sequelize.query("INSERT INTO users SET pseudo ='" + req.body.pseudo + "', mail='" + req.body.mail + "', mdp='" + hash + "', status='poisson';")
      .then(() => {
...
exports.login = (req, res) => {
  ...
  if ((regex.test(req.query.mdp) === true) && (mailRegex.test(req.query.mail) === true)){
    sequelize.query("SELECT * FROM users WHERE mail='" + req.query.mail + "' LIMIT 1;")
    .then(e => {
      let response= e[1][0]
...
exports.modify = (req, res) =>{
  if((regex.test(req.body.newName) === true) && (mailRegex.test(req.body.mail) === true)){ 
    sequelize.query("UPDATE users SET pseudo = '" + req.body.newName + "' WHERE id =" + req.body.id + " AND mail ='" + req.body.mail + "';")
